        ...
        try {
            ...
            const job = await createCronJob({
                ...
                task:
                    ...
                    '1. Use read_file to read ../identity/human.json — note the user\'s name, interests, projects, relationship.interactionCount.\n\n' +
                    '2. Use list_directory on ../memory/short-term/ then read_file on the 10 newest files — analyze patterns in topics, questions, tone.\n\n' +
                    '3. Update human.json: improve the "content" field with a 3-5 sentence summary of who the user is. Increment relationship.interactionCount by 1. Use write_file to save back to ../identity/human.json.\n\n' +
                    '4. Update soul.json: use read_file on ../identity/soul.json, then set memory.totalInteractions to the current interactionCount from human.json, add any new learnings to personality.learnings (max 8 entries total, keep the most relevant), update lastUpdated to current timestamp in ms. Use write_file to save back.\n\n' +
                    '5. Use write_file to create ../memory/long-term/identity-update-TIMESTAMP.json (use current Unix timestamp in ms as TIMESTAMP) with structure: { "type": "identity-update", "timestamp": <now ms>, "summary": "<2-3 sentences>", "changes": ["<change1>", "<change2>"] }\n\n' +
